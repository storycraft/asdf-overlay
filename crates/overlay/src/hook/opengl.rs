...
pub fn hook() -> anyhow::Result<()> {
    ...
    HOOK.get_or_try_init(|| unsafe {
        ...
        let wgl_swap_buffers =
            DetourHook::attach(addrs.swap_buffers as _, hooked_wgl_swap_buffers as _)?;
...
unsafe extern "system" fn hooked_wgl_swap_buffers(hdc: *mut c_void) -> BOOL {
    ...
    unsafe {
        mem::transmute::<*const (), WglSwapBuffersFn>(hook.wgl_swap_buffers.original_fn())(hdc)
    }
}
